-- 1. create_order_atomic: p_customer_phone 파라미터 추가
...
BEGIN
  ...
  INSERT INTO public.payments (
    order_id, store_id, status, method, amount, customer_name, customer_phone
  ) VALUES (
    v_order_id, p_store_id, 'waiting', p_method, v_total, p_customer_name, p_customer_phone
  );
...
-- 2. admin_set_payment_status: 재고 원복 시 max_stock 초과 방지
...
BEGIN
  ...
  SELECT store_role_at_least(p_store_id, 'manager') INTO v_allowed;
  IF NOT v_allowed THEN RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501'; END IF;

  SELECT status INTO v_old_order_status FROM public.orders
  WHERE id = p_order_id AND store_id = p_store_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'order_not_found' USING ERRCODE = '42704'; END IF;
  ...
  UPDATE public.orders SET status = v_order_status WHERE id = p_order_id;

  -- 테이블 상태 갱신
  IF p_new_status = 'completed' AND p_table_id IS NOT NULL THEN
    UPDATE public.tables SET status = 'empty' WHERE id = p_table_id AND store_id = p_store_id;
  ELSIF p_new_status = 'confirmed' AND p_table_id IS NOT NULL THEN
    UPDATE public.tables SET status = 'occupied'
      WHERE id = p_table_id AND store_id = p_store_id AND status = 'empty';
  END IF;

  -- 취소 시 재고 원복 (max_stock 초과 방지)
  IF p_new_status = 'cancelled' AND v_old_order_status <> 'cancelled' THEN
    UPDATE public.menus m
      SET stock = LEAST(m.stock + oi.quantity, m.max_stock),
          is_sold_out = false
    FROM public.order_items oi
    WHERE oi.order_id = p_order_id AND oi.menu_id = m.id AND m.store_id = p_store_id;
  END IF;

  INSERT INTO public.audit_log (store_id, user_id, actor, action, metadata)
  VALUES (p_store_id, auth.uid(), 'admin', 'payment.status',
...
-- 3. admin_set_order_status: 재고 원복 시 max_stock 초과 방지
...
DECLARE
  v_allowed boolean;
BEGIN
  IF p_new_status NOT IN ('pending','accepted','cooking','ready','served','cancelled') THEN
    RAISE EXCEPTION 'invalid_status' USING ERRCODE = '22023';
  END IF;

  SELECT store_role_at_least(p_store_id, 'kitchen') INTO v_allowed;
  IF NOT v_allowed THEN RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501'; END IF;
  ...
  -- served 시 payment도 completed
  IF p_new_status = 'served' THEN
    UPDATE public.payments SET status = 'completed' WHERE order_id = p_order_id;
  ELSIF p_new_status = 'cancelled' THEN
    UPDATE public.payments SET status = 'cancelled' WHERE order_id = p_order_id;
    -- 재고 원복 (max_stock 초과 방지)
    UPDATE public.menus m
      SET stock = LEAST(m.stock + oi.quantity, m.max_stock),
          is_sold_out = false
    FROM public.order_items oi
  ...
  INSERT INTO public.audit_log (store_id, user_id, actor, action, metadata)
  VALUES (p_store_id, auth.uid(), 'kitchen', 'order.status',
          jsonb_build_object('order_id', p_order_id, 'new_status', p_new_status));
