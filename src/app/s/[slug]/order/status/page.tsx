...
function OrderStatusPage() {
  ...
  const [lastStatus, setLastStatus] = useState<OrderStatus>('pending');
  const servingMode: ServingMode = store.serving_mode;

  /* Anon은 orders 테이블 직접 SELECT 불가 (RLS). RPC로 조회 + 10초 polling. */
  const fetchOrder = useCallback(async () => {
    if (!orderNo) return;
    const { data } = await supabase.rpc('get_customer_order', {
      p_store_id: store.id,
      p_order_number: orderNo,
    });
    const result = data as CustomerOrder | null;
    if (result) {
      setOrder(result);
      if (result.status === 'ready' && lastStatus !== 'ready') {
        setShowReadyPopup(true);
      }
      setLastStatus(result.status);
    }
  }, [orderNo, store.id, lastStatus]);
    ...
    <>
      ...
      <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
        ...
        <footer style={{
          ...
          <Link
            href={`/s/${store.slug}/order?table=${tableNumber}`}
