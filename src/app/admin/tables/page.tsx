...
export default function TablesPage() {
  ...
  const addBlock = async (kind: TableKind) => {
    ...
    const { data, error } = await adminApi<{ ok: boolean; data: Table }>('/api/admin/tables', {
      method: 'POST',
      body: {
        number: newNum,
        shape,
  ...
  const savePositions = async () => {
    ...
    const { error } = await adminApi('/api/admin/tables/batch', {
      method: 'PATCH',
      body: { items },
    });
    if (error) {
  ...
  const deleteSelected = async () => {
    ...
    const { error } = await adminApi('/api/admin/tables', {
      method: 'DELETE',
      body: { id: tid },
    });
    if (error) {
  ...
  const updateCapacity = async (tableId: number, capacity: number) => {
    ...
    await adminApi('/api/admin/tables', {
      method: 'PATCH',
